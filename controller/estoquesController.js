...
exports.getEstoques = async (req, res) => {
  try {
    const estoques = await Estoque.findAll();  // Não deve tentar acessar createdAt ou updatedAt
...
exports.getEstoqueById = async (req, res) => {
  try {
    const { id } = req.params;
    const estoque = await Estoque.findByPk(id);  // Não deve tentar acessar createdAt ou updatedAt
    if (estoque) {
